...
**Questions.** A question for the user is an item too, since one held only in a session's prompt is lost when that session clears or restarts, and the answer given in another session never reaches it. `ask` records it on the board, attached to the task it is about, before the session puts it to the user; every prime lists the open ones under `Waiting on you`, with who asked and how many writes ago. `answer` -- or `ekko --answer ID TEXT` at the terminal -- records the reply, once, which closes it, and the session that asked reads it in its next prime, after a /clear or a restart too, with how far the board moved before it came.
